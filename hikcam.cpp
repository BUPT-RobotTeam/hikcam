...
public:
    ...
    const cv::Mat &read()
    {
        ...
        cv::cvtColor(bayer, img, cv::COLOR_BayerRG2BGR);
...
PYBIND11_MODULE(hikcam, m)
{
    pybind11::class_<hikcam_info>(m, "hikcam_info")
        .def(pybind11::init<>())
        .def_readwrite("major_ver", &hikcam_info::major_ver)
        .def_readwrite("minor_ver", &hikcam_info::minor_ver)
        .def_readwrite("mac_addr", &hikcam_info::mac_addr)
        .def_readwrite("tlayer_type", &hikcam_info::tlayer_type)
        .def_readwrite("model_name", &hikcam_info::model_name)
        .def_readwrite("user_defined_name", &hikcam_info::user_defined_name);
    pybind11::class_<hikcam>(m, "hikcam")
        .def(pybind11::init<>())
        .def("enumerate", &hikcam::enumerate)
        .def("open", &hikcam::open)
        .def("start", &hikcam::start)
        .def("read", &hikcam::read)
        .def("stop", &hikcam::stop)
        .def("fps", &hikcam::fps);
}
